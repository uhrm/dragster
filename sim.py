...
def sim(fth, fcl, offset=0, verbose=0):
    ...
    t += 1
    # TODO: advance to <offset> frames
    yield FrameState(frame=t, status=0x0100, countdown=tm, time=tr, x=x, v=v, y=1, r=r, th=0, cl=0, rs=0)
    t += 1
    yield FrameState(frame=t, status=0x0100, countdown=tm, time=tr, x=x, v=v, y=1, r=r, th=0, cl=0, rs=0)
    t += 1
    tm = 159
    yield FrameState(frame=t, status=0, countdown=tm, time=tr, x=x, v=v, y=y, r=r, th=0, cl=0, rs=1)
    t += 1
    tm = max(tm-1, 0)
    yield FrameState(frame=t, status=0, countdown=tm, time=tr, x=x, v=v, y=y, r=r, th=0, cl=0, rs=1)
    t += 1
...
def write_script(script, fth, fcl, offset=0):
...
#        while t <  30: yield 1; t += 1
    ...
    def fcl():
        ...
        while t < 176: yield 1; t += 1
        while t < 212: yield 0; t += 1
        while t < 214: yield 1; t += 1
        while t < 268: yield 0; t += 1
        while t < 270: yield 1; t += 1
        while True:    yield 0; t += 1

    # write Stella debug script
    with open('dragster.script', 'w') as script:
